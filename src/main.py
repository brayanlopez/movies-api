...
def create_movie(movie: Movie) -> dict:
    movies.append(movie)
    return JSONResponse(content = {"mesage":"movie created succesfully"})
...
def update_movie(id: int, movie: Movie)-> dict:
    for item in movies:
        if item["id"] == id:
            ...
            return JSONResponse(content = {"mesage":"movie modified succesfully"})
...
def delete_movie(id: int)-> dict:
    for item in movies:
        if item["id"] == id:
            movies.remove(item)
    return JSONResponse(content = {"mesage":"movie deleted succesfully"})

@app.post("/login", tags=["auth"])
def login(user:User):
    if user.mail == "admin@gmail.com" and user.password == "admin":
        token_bytes = create_token(user.model_dump())
        token_str = token_bytes.decode()
        return JSONResponse(content = token_str, status_code =200)
